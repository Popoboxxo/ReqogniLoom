"""Migration 0005 backfills the Goal/MainGoal/Interview pairs into old rows.

Same caveat as ``test_grandfathered.py``'s migration block: the suite connects
as a BYPASSRLS role, so these tests do not prove the per-tenant
...
shape to ``0003``/``0004``, which were live-verified.
"""
...
_migration = importlib.import_module(
    "link_types.migrations.0005_backfill_goal_reference_pairs"
)

KEY = _migration.KEY

#: What the seeded rows looked like before the built-in gained the new pairs.
...
def _rewind(row):
    """Put a seeded row back into its pre-0005 shape."""
    row.definition_json = {
        **row.definition_json,
...
@pytest.fixture
def rewound_tenant():
    tenant = Tenant.objects.create(name="lt-bf-test", slug=f"lt-bf-{uuid.uuid4().hex}")
...
def test_migration_adds_the_new_pairs_to_global_and_workspace_rows(rewound_tenant):
    ...
    for row in (global_row, workspace_row):
        pairs = _pairs_of(row)
        assert set(_NEW_PAIRS) <= pairs
        # additive: the reference entities that were already there survive
