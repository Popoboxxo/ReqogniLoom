...
def test_decorator_rolls_back_on_error(tenant_a, workspace_a):
    ...
    def create_then_fail():
        ...
        # Induce real DB constraint violation (workspace is required)
        Artifact.objects.create(
            tenant=tenant_a, workspace=None, artifact_type="y"
        )
    ...
    with active_tenant(tenant_a):
        with pytest.raises(IntegrityError):
            with TransactionContextManager():
                ...
                # Induce IntegrityError
                Requirement.objects.create(
                    tenant=tenant_a, artifact=None, title="constraint on child 7"
                )
        assert Artifact.objects.count() == 0
