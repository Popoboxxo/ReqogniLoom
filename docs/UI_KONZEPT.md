...
| `RequirementEditors/RequirementTreeNode.tsx` | `ReqTraceLinkPanel` (Detail-Panel, keine eigene Route) | Ist kein Artefakt-Baum, sondern ein rekursiver Lazy-Explorer für den `derives-from`/`derived-by`-Trace-Link-Graphen: jeder Knoten lädt seine Kinder erst beim Aufklappen per eigenem API-Call (`tracelinksApi.listForArtifact`), mit eigenem Lade-/Fehlerzustand pro Knoten. `WorkspaceTree` erwartet dagegen ein vollständiges flaches `nodes[]`-Array, das vorab synchron aus `parent_id` (eine einzelne Eltern-Beziehung) zu einem Baum zusammengesetzt wird — Trace-Links bilden aber einen Graphen mit potenziellen Zyklen (deshalb der harte `MAX_DEPTH = 3`-Deckel) und mehreren gleichzeitigen Eltern/Kindern. Zusätzlich rendert jeder aufgeklappte Knoten zwei separat beschriftete Gruppen ("↑ Parents" / "↓ Children") unterhalb von sich selbst — `WorkspaceTree` kennt nur eine flache Position pro Knoten, kein Konzept mehrerer benannter Kind-Gruppen an einem Knoten. Virtualisierung (`virtualize`) setzt zudem eine im Voraus bekannte Zeilenzahl voraus, die hier erst nach asynchronem Nachladen entsteht. Eine erzwungene Migration würde entweder eager-fetching (Verhaltensänderung) oder eine neue Lazy-Loading-/Mehrfachgruppen-API in `WorkspaceTree` erfordern — beides außerhalb des Ziels "ein Artefakt-Baum". Bleibt bewusst ein eigenständiges, kleines Modul. | Task 4.3 (2026-08-03) |
| `ImpactView/ImpactView.tsx` (`ArtifactTreeNode`) | `/impact` | Task 4.4 (Virtualisierungs-Ratchet) prüfte diese Komponente als möglichen nächsten `WorkspaceTree`-Kandidaten, da sie eine eigene rekursive Baumdarstellung führt. Strukturell aber identisch zum oben dokumentierten `RequirementTreeNode`-Fall, nur mit noch mehr Gruppen: jeder Knoten lädt seine Trace-Links erst beim Aufklappen (`tracelinksApi.listForArtifact`), gruppiert die Treffer dynamisch nach `direction:link_type` (`groupEdges()` — beliebig viele benannte Untergruppen wie "→ derives from", "← satisfies", nicht nur zwei feste Parent/Child-Gruppen wie bei `RequirementTreeNode`), und führt eigene Zyklus-Erkennung über einen pfadbezogenen `visitedIds`-Set mit `MAX_DEPTH = 4`. `WorkspaceTree`s synchrones `nodes[]`/`parentId`-Modell bildet weder das asynchrone Nachladen noch die dynamische Mehrfachgruppierung ab. Bleibt aus denselben Gründen wie `RequirementTreeNode` bewusst ein eigenständiges Modul. | Task 4.4 (2026-08-15) |
| `canvas/CanvasEditor.tsx` | `/diagrams` (Sketch-Canvas) | Alle Farbwerte (~15 Hex-Literale) werden von Fabric.js als Canvas-2D-`fillStyle`/`strokeStyle` konsumiert (löst keine CSS Custom Properties auf — ein `var(...)`-String ist dort schlicht eine ungültige Farbe) und/oder als `<input type="color">`-Wert verwendet (verlangt laut HTML-Spec ein literales `#rrggbb`). Eine Migration auf `var(--color-*)` würde das eigentliche Canvas-Rendering brechen, kein reines Refactoring. Dauerhafte, strukturelle Ausnahme — kein Migrationsziel. | Theming Phase 2 Checkpoint 2 (2026-08-21); erneut bestätigt beim Contrast-Audit-Follow-up #140/#161 (2026-08-28) |
| `DiagramGraphEditor/GraphEdge.tsx` | `/diagrams` (Graph-Editor) | Setzt `style={pathStyle}` über eine gehoistete Style-Konstante, nicht über ein Objekt-Literal in der JSX-`style`-Prop — liegt damit außerhalb der Reichweite der ESLint-Regel `no-literal-color-in-inline-style` (die nur Literale innerhalb der `style={{...}}`-Attribut-Subtree prüft, siehe Regel-Docstring) und der zugehörigen Ratchet-Zählung. Der verbleibende rohe Hex-Wert (`containment`-Kantentyp, `#64748b`) hat keine ausreichend nahe `--palette-*`-Primitive ohne Kollision mit der `association`-Kante (`slate-600`) — siehe Kommentar in `tokens.css`. Dauerhafte Ausnahme. | Theming Phase 2 Checkpoint 2 (2026-08-21); erneut bestätigt beim Contrast-Audit-Follow-up #140/#161 (2026-08-28) |
| `shared/Dialog/` (`Dialog.tsx`, `Dialog.module.css`, `use-sheet-gestures.ts`) | alle Routen mit einem Dialog (~28 Aufrufer) | Issue #874 verlangt Bottom-Sheet-Verhalten auf Smartphone-Viewports und nennt dafür eine eigene `ResponsiveDialog.tsx`. Bewusst **nicht** als eigene Komponente umgesetzt: "zentriertes Modal → Bottom Sheet" ist eine reine Darstellung derselben Box, eine zweite Komponente hätte jeden der ~28 Aufrufer vor die Wahl zwischen zwei Primitives gestellt und die in #955/#873 gehärtete ARIA-/Fokus-Logik dupliziert. Das Verhalten liegt stattdessen als *eine* `@media (max-width: 640px)`-Regel im gemeinsamen Primitiv, sodass alle Dialoge mit einer Änderung profitieren (ARIA, Fokus-Reihenfolge und Portal-Ziel sind bei jeder Breite unverändert). Zwei weitere Abweichungen vom Issue-Wortlaut: (a) Touch-Ziele wachsen auf `--btn-h-lg` (44px) statt auf 48px-Literale — die Button-Skala hat keinen 48px-Spross, ein Literal verstieße gegen die Konvention "keine hardcodierten Größen"; (b) die 640px-Grenze ist kein dokumentierter `--bp-*`-Layout-Breakpoint (`--bp-md` = 768px), sondern der Wert, den `Dialog.module.css` schon vor #874 für Mobil verwendete und den das Issue genau so nennt. | Issue #874 (2026-09-16) |
