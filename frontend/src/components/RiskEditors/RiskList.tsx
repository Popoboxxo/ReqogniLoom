  ...
  return (
    <div>
      ...
      {items.length === 0 ? (
        ...
        <p data-testid="risk-list-no-matches" style={{ color: 'var(--color-text-muted)', fontSize: 'var(--font-size-sm)' }}>{t('editor.noMatches', 'No matches found.')}</p>
      ) : (
        <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
          {visible.map((it) => {
            ...
            return (
              ...
              >
                <a href={`/risks/${it.id}`} onClick={(e) => { e.preventDefault(); navigate(`/risks/${it.id}`); }}
