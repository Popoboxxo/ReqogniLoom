...
## 9. W2-Slice-Nachweis (2026-09-26)

**Revisionsstand zum Schreibzeitpunkt (Lesehinweis für den ganzen Abschnitt).** Der W2-Slice ist zum Zeitpunkt dieses Eintrags **noch nicht committet**; eine W2-Commit-SHA existiert daher **nicht** und wird in diesem Register **nicht geraten**. Festgelegt und für den nachfolgenden Git-Durchgang verbindlich ist stattdessen die **Commit-Struktur**: **drei** bounded Commits auf `feat/audit-w2-atomicity-concurrency` (Basis `origin/main` = `acde772e`), anschließend **ein** PR gegen `main`. Die Zuordnung dieses Nachweises erfolgt damit zunächst über **Branch + Commit-Messages + PR-Revision**; die abschließende, **datierte Provenienz-Auflösung** mit den exakten SHAs bleibt der nachfolgenden Nachtragsarbeit vorbehalten — nach dem im Register bereits etablierten Muster aus [7.4](#74-auflösung-der-provenienz-2026-09-25) und 7.5, wo eine unaufgelöste Provenienz ausdrücklich als offene Messung geführt und erst später datiert nachgetragen wurde. Sämtliche Aussagen dieses Abschnitts, die eine fehlende Revision nennen, sind als **Aussage über den Schreibzeitpunkt** zu lesen und werden durch die geplanten Commits **nicht** automatisch aufgehoben, sondern erst durch die datierte Provenienz-Auflösung.

| Feld | Festgehaltener Stand |
|---|---|
| **Datum** | 2026-09-26 (Dokumentationsdatum). Einziges im Working Tree datierbares Artefakt: der Generierungszeitstempel in `backend/application/migrations/0027_alter_domaineventoutbox_event_type.py:1` — „Generated by Django 6.1.1 on 2026-09-25 22:57". |
| **Scope** | W2-Teilmenge des Plans `docs/superpowers/plans/2026-09-24-motivated-improvement-plan.md` §7.4 („W2 — Workflow, Atomizität und fachlicher Konsistenzkern") mit den Arbeitspaketen 1, 2 (Lock-/Audit-/Outbox-Teil), 3, 4, 5 und 7 (soweit Audit-/Outbox-Seam). **Nicht** Teil dieses Nachweises: Arbeitspaket 6 (Workspace-Sprache / `CR-13`), Arbeitspaket 8 (Trennung der asyncen Provider-/Chat-Grenze), der Constraint-Teil aus Arbeitspaket 2, der Deferred-D1-Folgeschritt. |
| **Status** | Zum Schreibzeitpunkt **ohne Commitbindung** — es existiert **keine** W2-SHA. Die Commitbindung ist für den nachfolgenden Git-Durchgang als **drei** bounded Commits festgelegt (siehe [9.1](#91-revision-branch-und-commitbindung)). Tests vorhanden und grün, aber **kein Track wird als `VERIFIZIERT` markiert** — Begründung in [9.1](#91-revision-branch-und-commitbindung) auf der Grundlage der Abschlussregel in [§6](#6-pflege--und-abschlussregel). DoD-Verdikt: `TEILWEISE` / `APPROVED_WITH_NOTES`. **Kein** §11.3-Abbruchkriterium eingetreten. |
| **Verhältnis zu den Abschnitten 1–8** | Die Abschnitte 1–8 sind **inhaltlich unverändert**; insbesondere bleiben die Statuszellen der kanonischen Tracks in [Abschnitt 2](#2-kanonisches-finding-trackregister), die Widerspruchstabelle in [4](#4-widersprüche-und-offene-reconciliation-entscheidungen) und die Abschlussregel in [6](#6-pflege--und-abschlussregel) unangetastet. W2-Status ist — wie W1 in [8.4](#84-was-je-audit-track-tatsächlich-geschlossen-wurde) — ausschließlich eine **datierte Annotation in diesem Abschnitt**. |
| **Platzierung** | Dieser Abschnitt steht am Ende der inhaltlichen Gliederung. Der terminale `STATUS:`-Block am Dateiende bleibt der **historische Abschlussvermerk des Audits vom 2026-09-24** und wird weder ergänzt noch umformuliert; er ist **nicht** der W2-Abschlussvermerk. |

> **Fortschreibung der Revisionsaussagen (2026-09-26).** Der vorstehende „Revisionsstand zum Schreibzeitpunkt" und die Revisionszeilen in [9.1](#91-revision-branch-und-commitbindung) beschreiben den Stand **vor** dem Commit-Durchgang. Dieser ist inzwischen erfolgt: der W2-Slice ist **committet, gepusht und revisionsgebunden**, und **PR #1073** existiert. Die betroffenen Revisionszellen in [9.1](#91-revision-branch-und-commitbindung) und [9.13](#913-geplanter-pull-request) sind deshalb **datiert fortgeschrieben** und nennen die tatsächliche Revision; die Unberührt bleiben die **gemessenen Evidenzzeilen** der Abschnitte 9.2–9.9, 9.11 und 9.12; in [9.10](#910-nicht-nachgemessene-größen-und-offene-messungen) sind ausschließlich die Zeilen 6 und 7 fortgeschrieben, alle übrigen offenen Messungen bleiben unverändert. Die **abschließende, datierte Provenienz-Auflösung** für W0/W1/W2 — nach dem Muster aus [7.4](#74-auflösung-der-provenienz-2026-09-25) und [7.5](#75-weiterführung-der-provenienz-auflösung-2026-09-25-pr-1071) — steht in [9.17](#917-provenienz-nachtrag-w0w1w2-2026-09-26-pr-1073). **W2 bleibt `TEILWEISE VERIFIZIERT`:** Die Revisionsbindung liegt jetzt vor, die Exit-Kriterien sind aber **nicht vollständig** erfüllt, und die Statuszuschreibung der einzelnen Tracks ist **noch nicht** erfolgt.

### 9.1 Revision, Branch und Commitbindung

| Feld | Festgehaltener Stand |
|---|---|
| Arbeitsbranch | `feat/audit-w2-atomicity-concurrency` (verifiziert: `git rev-parse --abbrev-ref HEAD`) |
| Basis / Abzweigpunkt | `acde772e7def60dfe70b1ee0188ce28f152d7ee5` — Merge-Commit von **PR #1071** („Merge pull request #1071 from Popoboxxo/feat/audit-w1-closeout", verifiziert per `git log -1 acde772e`). Das ist der Stand von `origin/main`, von dem der Arbeitsbranch abgezweigt ist. |
| `HEAD` zum Schreibzeitpunkt | **identisch** mit `origin/main` = `acde772e` (verifiziert). **Nach** dem Commit-Durchgang ist `HEAD` **nicht** mehr `acde772e`; `acde772e` bleibt dann der **Parent des ersten W2-Commits** und ist in dieser Zeile als **Basis**, nicht als `HEAD`, zutreffend. |
| **`HEAD` nach dem Commit-Durchgang** (Fortschreibung 2026-09-26) | **`d4d912bf595e821820ea6989342e272e888e9a3a`** (verifiziert per `git rev-parse HEAD`). `origin/main` ist **unverändert** `acde772e7def60dfe70b1ee0188ce28f152d7ee5` (verifiziert per `git rev-parse origin/main`) und damit weiterhin die **Basis**, nicht der `HEAD`. |
| Branch ist **gepusht** (Fortschreibung 2026-09-26) | `origin/feat/audit-w2-atomicity-concurrency` = **`d4d912bf…`** und ist damit **identisch** mit dem lokalen `HEAD` (verifiziert per `git rev-parse --abbrev-ref --symbolic-full-name @{u}` und `git rev-parse origin/feat/audit-w2-atomicity-concurrency`). Der Slice ist also **committet und gepusht**. |
| Commitbindung des W2-Slices | **Zum Schreibzeitpunkt keine** — es existiert bewusst **keine** SHA: es wurde **nicht** committet, **nicht** gepusht, **kein** PR eröffnet, **kein** Branch-Wechsel über den Arbeitsbranch hinaus vorgenommen. |
| **Commitbindung nach dem Durchgang** (Fortschreibung 2026-09-26) | **Verbindlich, revisionsgebunden.** Drei Commits auf `feat/audit-w2-atomicity-concurrency`, **in dieser Reihenfolge** (alle drei verifiziert per `git log -1 <sha>`):<br>1. `6f145c87428971406c62c0d69793a65f7a1feab3` — „fix(CR-08): enforce expected_version through the workflow lock"<br>2. `14fc05e58f15eb567e06403c3f3efd0a474ae2fe` — „fix(CR-06/CR-07/CR-05): serialise and audit interview formalization"<br>3. `d4d912bf595e821820ea6989342e272e888e9a3a` — „fix(CR-09/CR-10): make global definition propagation atomic and orphan-safe"<br>Basis: `acde772e` (Merge von **PR #1071**). Pull Request: **#1073**, **offen** gegen `main` (verifiziert per `gh pr view 1073`). Diff gegen die Basis: **30 Dateien, +4858 / −153** (verifiziert per `git diff --shortstat acde772e HEAD`). Ein **Merge-SHA existiert nicht**, weil der PR offen ist. |
| Festgelegte Commit-Struktur | **Drei** bounded Commits in dieser Reihenfolge (Conventional Commits, erste Zeile jeweils max. 72 Zeichen, exakt so beabsichtigt):<br>1. `fix(CR-08): enforce expected_version through the workflow lock`<br>2. `fix(CR-06/CR-07/CR-05): serialise and audit interview formalization`<br>3. `fix(CR-09/CR-10): make global definition propagation atomic and orphan-safe`<br>**Erfüllung:** Die Struktur wurde **eins zu eins** umgesetzt — Commit-Betreffe und Reihenfolge stimmen mit [9.11](#911-working-tree-inventar-des-w2-slices) überein. |
| Zuordnung der 29 Slice-Pfade | in [9.11](#911-working-tree-inventar-des-w2-slices) als **geplante** Zuordnung ausgewiesen. Sie ist **aus dem Dateiinhalt abgeleitet** (Grep über die Working-Tree-Diffs), **nicht** aus einer Git-Historie, und wird nach dem Commit-Durchgang je Commit über `git show --stat <sha>` verifiziert. |
| Nachprüfbarkeit vor der SHA | Bis die SHAs vorliegen, ist die Zuordnung revisionsgebunden über **Branch + die drei Commit-Messages + die PR-Revision**. Eine SHA wird **nicht** vorweggenommen. **Überholt seit 2026-09-26:** Die SHAs liegen vor (siehe [9.1](#91-revision-branch-und-commitbindung)); die Zuordnung ist damit **unmittelbar** revisionsgebunden und je Commit über `git show --stat <sha>` nachprüfbar. |
| Staging | `git diff --cached --name-only` ist **leer** (verifiziert zum Schreibzeitpunkt). Der Commit-Durchgang konsumiert den Index; der Zeilenwert „leer" besteht daher auch danach — er ist **kein** Nachweis über den Slice-Inhalt. |
| **Working Tree nach dem Durchgang** (Fortschreibung 2026-09-26) | `git status --porcelain` ist **leer** (verifiziert): der Slice ist vollständig committet, es gibt keine untracked Restdatei und keinen uncommitteten Diff. Das ist **kein** Nachweis über den Slice-Inhalt, sondern nur über den Commit-Abschluss. |
| Working Tree | **18 tracked geänderte** Slice-Pfade + **11 untracked** Slice-Pfade = **29** (verifiziert: `git status --porcelain=v1 -uall`; für die Slice-Pfade `git diff --stat` = „18 files changed, 1096 insertions(+), 153 deletions(-)"). **Zählhinweis:** `git status` zeigt **19** tracked geänderte Pfade, weil **dieses Register selbst** Teil des Diffs ist (Abschnitt 9); seine Zeilenzahl wächst mit jeder Fortschreibung dieses Abschnitts. **Nachrechnung:** `git diff --stat` gesamt **minus** dem Diff-Beitrag dieses Registers ergibt die Slice-Werte „18 files changed, 1096 insertions(+), 153 deletions(-)" — die Slice-Zahl **18 / 11 / 29** ist davon unberührt. **Dieses Register ist Dokumentation, kein W2-Slice-Pfad**, und geht in keine Slice-Zahl dieses Abschnitts ein. |
| Nachweis des Working Trees | `git status --porcelain=v1 -uall`; `git diff --name-only` |

> **Warum kein Track `VERIFIZIERT` ist — die tragende Begründung dieses Abschnitts.** Die Abschlussregel in [§6](#6-pflege--und-abschlussregel) verlangt für `VERIFIZIERT` ein ausführbares Test-/CI-Artefakt, **Commit/Revision**, Umgebung, Ergebnis und gegebenenfalls Abweichung. Zum Schreibzeitpunkt existiert **keine** W2-Revision. Damit fehlt die Revisionsbindung, und die Evidenz in [9.3](#93-ausgeführte-kommandos-und-ergebnisse) hängt an einem flüchtigen Working Tree. **Die geplanten Commits heben diese Aussage nicht automatisch auf:** Sie beschreibt den Stand dieses Eintrags und bleibt bis zur datierten Auflösung stehen. Nach der W1-Präzedenz — Abschnitt 7 war ausdrücklich als „uncommitted" markiert und [7.4](#74-auflösung-der-provenienz-2026-09-25) löste die Provenienz erst nachträglich und **datiert** über Commit/PR auf — gilt für W2 dasselbe: **Tests existieren und sind grün, aber solange die Revision nicht vorliegt, gilt kein Track als verifiziert.** Die Zuordnung erfolgt künftig über **Branch + Commit-Messages + PR-Revision**; die **abschließende Statuszuschreibung** (`VERIFIZIERT` oder weiterhin `OFFEN`) erfolgt in einer **datierten Provenienz-Auflösung**, sobald die SHAs vorliegen. **Nachtrag 2026-09-26:** Die SHAs liegen vor, die Zuordnung ist damit **revisionsgebunden**. **Trotzdem wird kein Track als `VERIFIZIERT` markiert** — die Revisionsbindung ist nur **eine** der Bedingungen aus [§6](#6-pflege--und-abschlussregel). Die W2-Exit-Kriterien sind **nicht vollständig** erfüllt (siehe [9.8](#98-exit-kriterien-des-w2-plans-erfüllt--teilweise--nicht-erfüllt) und [9.9](#99-negativraum-was-ausdrücklich-nicht-geschlossen-ist)); der Track-Status bleibt damit **`TEILWEISE VERIFIZIERT`**. Die Statuszuschreibung je Track ist ein **eigener, datierter Nachtrag** und ist zum Schreibzeitpunkt **nicht** erfolgt. **Diese Präzisierung ändert die Nachweisregel des Registers nicht:** Bewertungsmaßstab bleibt ausschließlich [§6](#6-pflege--und-abschlussregel); es wird **kein** neuer Evidenzklassifikations-Standard eingeführt und keine Statuszelle in [Abschnitt 2](#2-kanonisches-finding-trackregister) umgeschrieben.

> **Bewusste Abgrenzung — nicht Teil dieser Welle.** Ein **getrenntes, unabhängiges** Meta-Config-Vorhaben ist **kein** W2-Artefakt: Commit `22aeae4e` („chore: refresh project metadata"), Branch `chore/refresh-project-metadata`, betroffene Datei `.meta-config/project.yaml`. Verifiziert per `git show --stat 22aeae4e`: der Commit berührt **genau eine** Datei (149 insertions / 119 deletions), und der Branch existiert lokal wie auf `origin`. Eine **PR-Nummer** wird hier **nicht** genannt — sie ist eine GitHub-seitige Angabe und per lokalem `git` **nicht** verifizierbar. `.meta-config/project.yaml` erscheint **nicht** im Working-Tree-Inventar von W2 (verifiziert über `git status --porcelain=v1 -uall`) und geht in **keine** der W2-Zahlen in [9.3](#93-ausgeführte-kommandos-und-ergebnisse) ein.

### 9.2 Verifizierte Umgebung

| Feld | Festgehaltener Stand |
|---|---|
| Host | Windows |
| Datenbank | **echtes PostgreSQL 16.15** (Debian 16.15-1.pgdg12+2), live gemessen per `select version()` |
| Django | **6.1.1** |
| Python (Container) | **3.12.14** |
| pytest | **9.1.1** |
| ruff (Host) | **0.16.9** — **Abweichung:** CI pinnt `ruff==0.16.5` (`.github/workflows/ci.yml:106`, Installzeile; Gate-Aufruf in `.github/workflows/ci.yml:115`). Die lokale `I,F,E`-Aussage ist damit eine **Superset-Prüfung**, keine byte-identische CI-Reproduktion — identisch zur in [8.6](#86-verbleibendes-restrisiko-bewusst-offen-geführt) Punkt 9 und [8.1](#81-verifizierte-umgebung) vermerkten Abweichung. |
| Test-Runner | `docker-compose -f deploy/docker-compose.yml -f testing/docker-compose.test.yml --project-directory . run --rm backend-test …` mit `DJANGO_SETTINGS_MODULE=reqogniloom.settings_test` |
| Pfadargumente | In-Container-`rootdir` = `/app`; daher sind Pfadargumente **ohne** `backend/`-Präfix zu übergeben. |
| Baseline vor der Slice-Arbeit | `workflow/tests/test_lifecycle_manager.py` = **16 passed**, Exit 0 |

### 9.3 Ausgeführte Kommandos und Ergebnisse

Alle Zahlen und Exit-Codes in dieser Tabelle sind **übergebene Messwerte des Slice-Laufs**. Sie wurden für diesen Nachtrag **nicht** nachgemessen — es wurde bei der Dokumentation **kein Test, kein Lint und kein Migrationstest ausgeführt**. Sie stehen hier als Berichtswerte und werden in [9.9](#99-negativraum-was-ausdrücklich-nicht-geschlossen-ist) und [9.10](#910-nicht-nachgemessene-größen-und-offene-messungen) entsprechend eingeordnet.

| Gate | Ergebnis | Exit |
|---|---|---|
| neue + geänderte W2-Testdateien (**11** Pfade) | **167 passed** | **0** |
| breite Regression `workflow/ application/ mcp_server/ rest_api/` | **6330 passed, 15 skipped** | **0** |
| unabhängige Nachmessung derselben vier Apps | **6299 passed, 15 skipped** | **0** |
| `manage.py check` | „System check identified no issues (0 silenced)" | **0** |
| `manage.py makemigrations --check --dry-run` | „No changes detected" | **0** |
| `pytest mcp_server/tests/test_tool_manifest_drift.py` + `test_export_tool_manifest.py` | **5 passed** | **0** |
| `ruff check . --select=F821,F822` (das tatsächliche CI-Gate, `.github/workflows/ci.yml:115`) | keine Findings | **0** |
| `ruff check --select=I,F,E` über die Slice-Dateien | Baseline **39 → 38**, **added: none** | **0** |
| `git diff --check` | keine Findings | **0** |
| Whitespace-/Tab-/Final-Newline-Scan über die untracked Dateien | **0** Findings | **0** |
| Secret-Scan (alle **28** Slice-Dateien, tracked + untracked) | **0** Findings (Endstand; Ausgangslage 4 Findings, siehe [9.4](#94-aufgedeckte-diskrepanzen-und-fehlversuche)) | **0** |

**Zwei der obigen Zahlenpaare sind bewusst **nicht** zusammengeführt** und werden unglättiert nebeneinander geführt: `6330` und `6299` (siehe [9.4](#94-aufgedeckte-diskrepanzen-und-fehlversuche)) sowie `253`/`116` (siehe dort). Dieses Register ersetzt **keine** der gemeldeten Größen durch eine andere.

### 9.4 Aufgedeckte Diskrepanzen und Fehlversuche

Nichts in diesem Unterabschnitt wurde bereinigt, geglättet oder weggelassen.

**a) Zwei unabhängige Messungen derselben vier Apps mit unterschiedlichem Ergebnis.** Die breite Regression über `workflow/ application/ mcp_server/ rest_api/` meldet **6330 passed, 15 skipped**; eine **unabhängige Nachmessung derselben vier Apps** meldet **6299 passed, 15 skipped** — eine Differenz von **31** Tests bei identischer Skip-Zahl und Exit 0. **Dieses Register erklärt die Differenz nicht und wählt keine der beiden Zahlen.** Naheliegend, aber **nicht belegt**, ist eine unterschiedliche Selektion/Parametrisierung zwischen den beiden Läufen; das ist eine Vermutung, keine Feststellung. Beide Werte bleiben als Messstand stehen.

**b) Migrationsbasiszahl für die E501-Entscheidung weicht von der Nachmessung ab.** Zur Begründung des nicht behobenen E501 wurde die Größenordnung „von **253** Migrationsdateien tragen **116** ein E501" herangezogen. Nachgemessen über `git ls-files 'backend/*/migrations/*.py'`: **270** tracked Migrationsdateien insgesamt, davon **16** `__init__.py` und **254** echte Migrationsdateien; die neue Datei `0027_alter_domaineventoutbox_event_type.py` ist **untracked** und taucht in `git ls-files` nicht auf, mit ihr wären es **255**. Die genannte Basis **253** lässt sich aus dieser Zählung **nicht** reproduzieren. Die Differenz wird **nicht** aufgelöst und **nicht** durch eine der gemessenen Zahlen ersetzt; sie ist eine **offene Messung** (siehe [9.10](#910-nicht-nachgemessene-größen-und-offene-messungen)). Die **E501-Aussage selbst** — 1 Befund in der generator-erzeugten Migration — ist davon unberührt.

**c) `ruff --select=I,F,E`: 1× E501 in einer generator-erzeugten Migration, bewusst nicht behoben.** Befund: **1× E501** in `backend/application/migrations/0027_alter_domaineventoutbox_event_type.py` (die einzeilige `choices`-Liste, Zeile 16 — im Repo-Stand sichtbar). **Bewusst nicht behoben**, weil ein Hand-Edit die Datei aus der Generator-Ausgabe divergieren ließe. Begründung: von den Migrationsdateien trägt eine große Mehrheit ein E501, **0** nutzen ein `noqa`; die Entscheidung stützt sich auf die in (b) genannte, nicht aufgelöste Basiszahl. **Falls das Gate je 0 Findings verlangt, wäre eine `per-file-ignores`-Regel für `*/migrations/*` sauberer als ein Edit** — das ist als Option benannt, **nicht** umgesetzt.

**d) Secret-Scan: erst 4 Findings, dann 0 — und eine Optimismus-Aussage, die ein echter Regressionstest widerlegte.** Die vorbestehende Deklaration „0 Findings" war **in ihrer naheliegenden Lesart falsch**: Sie erfasste nur die **hinzugefügten Zeilen des tracked Diffs** und ließ die **neuen Dateien** aus. Vor der Auflösung meldete der Scan **4 Befunde**:

| Datei | Befunde |
|---|---|
| `backend/mcp_server/tests/test_interview_tool_group.py` | 1 |
| `backend/mcp_server/tests/test_interview_tool_group_formalize_parity.py` | 1 |
| `backend/rest_api/tests/test_transition_expected_version_cr08.py` | 2 |

**Die Aufdeckung ist keine Fußnote, sondern ein Befund:** Die **DoD-Validierung** maß den Secret-Scan mit **4 Findings**, **nicht** mit 0 — ein **echter Regressionstest**, der einer Optimismus-Aussage im Entwicklungsbericht **widersprach**. Auflösung über die dokumentierte **`_SAFE_PATTERNS`-Allowlist** des Scanners, nach dem bereits committeten Präzedenzfall `backend/rest_api/tests/test_readonly_and_unknown_field_rejection_915_916.py:53` (`_SECRET = "jwt-signing-key-placeholder"`). Das **Scanner-Muster matcht weiterhin**; nur die Allowlist hebt den Befund auf — **nichts wurde obfuskiert, um die Erkennung zu umgehen**. Endstand: **0 Findings**.

**e) Regressionserkennung im Fix-Durchlauf.** Nach der Umstellung von `delete_state` auf eine **gesperrte Instanz** zeigte die Response **noch den gelöschten State**; **4 Tests** fielen darauf. Korrigiert. Der Zwischenstand war also rot und ist Audit-Trail, kein bereinigter Nebenschauplatz.

**f) `m7` des Reviews erwies sich als nicht erreichbar — der Test wurde auf die tatsächlich erreichbare Hälfte umgestellt.** Die vermutete Variante scheiterte mit `IntegrityError`, weil `(tenant_id, workspace_id, item_type)` **bereits unique** ist: die Constraint `uq_wedef_tenant_ws_type` existiert (verifiziert in `backend/workflow/models.py:167` und `backend/workflow/migrations/0001_initial.py:100`). Behoben wurde die **tatsächlich erreichbare** Hälfte: **eine** Zeile, deren `preset` vom Loop-Preset abweicht. Die **Reachability** ist im Docstring festgehalten. Die ursprünglich vermutete Variante wird **nicht** als belegt geführt.

**g) Eine Testsuite wurde als nicht vakuum-grün geprüft — vier Negative Controls, alle beweislastend.** Die Test**logik** wurde geprüft, nicht nur der Bericht:

| Negative Control | Wirkung |
|---|---|
| `atomic()` aus `_persist` entfernt | Fault-Injection-Tests werden **rot** |
| Orphan-Gate entfernt | Orphan-Tests werden **rot** |
| Lock-Read entfernt | Concurrency-Test wird **rot** — **prä-fix 2× „ok", post-fix 1×** |
| `abandon`-Lock entfernt | Serialisierungs-Tests werden **rot** |

**Kein vakuum-grüner Test wurde gefunden.**

**h) Zwei echte Fallstricke wurden aktiv vermieden — nicht zufällig, sondern bewusst.**
1. **Thread-Tests brauchen `django_db(transaction=True)`**, weil die zweite Verbindung sonst die Zeilen der ersten **nicht** sieht. Ein Thread-Test ohne dieses Marker wäre grün, **ohne** die behauptete Mehrverbindungs-Wirkung zu prüfen.
2. Eine **Interview-Fixture ohne provisionierte Workflowdefinition** lässt den **verschluckten** Transition-Fehler wie einen **Erfolg** aussehen. **Beide** Thread-Fixtures provisionieren die Definition deshalb **ausdrücklich**.

**i) Pfad-/Zählgrundlage des Secret-Scans weicht vom Working Tree ab.** Der Scan umfasst laut Bericht **28** Slice-Dateien (tracked + untracked). Der verifizierte Working Tree umfasst zum Zeitpunkt dieses Nachtrags **29** Pfade (**18** tracked + **11** untracked, siehe [9.1](#91-revision-branch-und-commitbindung)). Die Differenz von **1** wird **nicht** aufgelöst; sie kann eine Scan-Auswahl-Abgrenzung (z. B. eine bewusst ausgenommene Doku-Datei) oder ein Zählfehler sein — **beides ist hier eine Vermutung, kein Befund.** Festgehalten wird nur: die Scan-Grundlage ist **nicht 1:1 identisch** mit dem Working-Tree-Inventar dieses Nachtrags. Ergänzend: die für diesen Nachtrag übergebene Dateiauswahl nennt **zwei** geänderte tracked Testpfade **nicht**, die im Working Tree verifiziert geändert sind — `backend/mcp_server/tests/test_interview_tool_group.py` (tritt in (d) als Secret-Befund auf) und `backend/rest_api/tests/test_views.py`. Die Auswahl wird daher als **Auswahl, nicht als vollständiges Inventar** geführt.

### 9.5 Code-Review- und Validierungsbefunde

**Code-Review (tatsächlich durchgeführt):**

| Feld | Ergebnis |
|---|---|
| Verdikt | **`CHANGES_REQUESTED`** |
| Blast-Level | **SIGNIFICANT** |
| Befunde | **2 BLOCKER + 5 MAJOR + 8 MINOR** |
| Neue ruff-Findings | **0** |
| Abwicklung | **Beide BLOCKER, alle MAJOR und alle MINOR** wurden anschließend behoben. |

**Die vier last-bearing Reparaturen:**

1. **CR-10:** Die Reparatur überschrieb `workflow_json` **unbedingt** und konnte **Live-Items stranden**. Jetzt: **Orphan-Vorprüfung vor dem `.update(...)`**; der Nicht-repariert-Report führt den Marker **`would-orphan-live-item`** (verifiziert in `backend/workflow/management/commands/provision_workflow_definitions.py:42,198`).
2. **CR-09b:** Das Orphan-Gate war **Check-then-Act über zwei Transaktionen** und damit **nicht fail-closed**. Jetzt: **ein** `transaction.atomic()` mit `select_for_update()` auf der **globalen Definitionszeile** (verifiziert in `backend/workflow/global_definition_store.py:366–367`).
3. **Der `revalidator`-Hook** war **nachweislich unerreichbar**, und seine Docstring-Aussage **widersprach** `workflow_facade.py`. **Hook und widersprüchliche Doku entfernt** — verifiziert: `git grep "revalidator" -- backend/` liefert **0 Treffer**.
4. **`abandon()` war nicht gegen `formalize()` serialisiert.** Jetzt über **denselben Lock**.

**Weitere Review-Befunde mit Beanstandungscharakter:**

- Die **Workflow-Revision war für REST-Clients nicht discoverable.** Jetzt in **beiden** Responses enthalten, mit **ausdrücklicher Warnung**, dass `If-Match` auf dieser Ressource die **Workflow-Revision (409)** meint und **nicht** die Entitäts-ETag (**412**) — verifiziert in `backend/rest_api/mixins/workflow_transitions.py:359–376`.

**Review-Ehrlichkeit — ausdrücklich festgehalten:** Für den entfernten `revalidator`-Hook wurde **kein** Test geliefert. Das ist **konsistent** damit, dass er unerreichbar und damit untestbar war. Die **Negative Control** aus [9.4](#94-aufgedeckte-diskrepanzen-und-fehlversuche) (g) belegte den **Lock-Read-Fix**, **nicht** eine Revalidierungsschicht. Es wird hier **keine** Revalidierungsschicht behauptet.

**DoD-Validierung (tatsächlich durchgeführt):**

| Feld | Ergebnis |
|---|---|
| Verdikt | **`TEILWEISE` / `APPROVED_WITH_NOTES`** |
| Negativtests | **5 von 6** W2-Negativtests beweiskräftig, **einer fehlt** (siehe [9.7](#97-negativtests-des-w2-plans-welcher-belegt-ist-und-welcher-fehlt)) |
| §11.3-Abbruchkriterium | **Kein** eingetreten — siehe [9.8](#98-exit-kriterien-des-w2-plans-erfüllt--teilweise--nicht-erfüllt) |

### 9.6 Was je Track tatsächlich belegt wurde

Die Statuszellen der Tracks in [Abschnitt 2](#2-kanonisches-finding-trackregister) bleiben **unverändert**. Die folgende Tabelle ist eine **datierte W2-Annotation** und **schließt keinen Track**.

| Track | In W2 tatsächlich belegt | Status |
|---|---|---|
| `CR-08` | Tests vorhanden und grün: **Zwei-Verbindungen-/History-Negativtest**, **409-statt-500**, **MCP-Parität**. Dateien: `backend/workflow/tests/test_transition_concurrency_cr08.py`, `backend/rest_api/tests/test_transition_expected_version_cr08.py`, `backend/mcp_server/tests/test_transition_version_conflict_cr08.py`. Die Versionsrevision ist für REST-Clients discoverbar; der `If-Match`-Konflikt (409 vs. 412) ist im Code ausdrücklich gewarnt. | **bleibt OFFEN mangels Revision** |
| `CR-06` | **Single-Formalize-Lock + Status-Recheck + Race-Negativtest.** Die **DB-Constraint ist bewusst deferriert** — benannter Folgeschritt, dokumentiert in `backend/application/interview_service.py` (Docstring `_lock_in_progress_session`) und in Plan §7.4. | **bleibt OFFEN** |
| `CR-07` | **Audit-Seam** in `single` / `multi` / `abandon`; **neues Outbox-Event**; der **Fehlerfall erzeugt kein Erfolgs-Event**. Verifiziert: `INTERVIEW_ABANDONED` in `backend/application/models.py:111` und `backend/application/interview_service.py:1560`. | **bleibt OFFEN** |
| `CR-05` | **MCP-Parität** (Multi-Formalize jetzt ausführbar). **Ausdrücklich nur die Teilmenge Multi.** Die Parität wird **MCP-seitig gegen den vorbestehenden REST-Test** belegt — **nicht** durch eine **gemeinsam erzeugte Response**. Datei: `backend/mcp_server/tests/test_interview_tool_group_formalize_parity.py`. | **bleibt OFFEN** |
| `CR-09` | **Atomizität + Orphan-Gate**, beide per **Fault-Injection** bzw. **Negativtest** belegt. **Scope ausdrücklich begrenzt** auf **fehlende Atomizität** zwischen globaler und abgeleiteter Wahrheit plus **fehlenden Live-Item-/Orphan-Schutz**. Es wird **keine** selektive Row-Teilpropagation behauptet. **Bandbreite-Korrektur:** maßgeblich bleibt [`12-evidence-validation-and-corrections.md`](12-evidence-validation-and-corrections.md); der belegte Defekt ist **ausschließlich** die fehlende Atomizität plus der fehlende Live-Item-/Orphan-Schutz. Datei: `backend/workflow/tests/test_global_definition_store_atomicity.py`, `backend/rest_api/tests/test_global_default_orphaned_state_409.py`. | **bleibt OFFEN** |
| `CR-10` | **Provisioning-Repair** mit **Orphan-Vorprüfung**, **Fail-safe** und **benanntem Nicht-repariert-Report** (`would-orphan-live-item`). Datei: `backend/workflow/tests/test_provision_workflow_definitions.py`. | **bleibt OFFEN** |
| `CR-13` | **Weder geschlossen noch ausgewiesen** — **kein Test, keine Implementierung**. | **bleibt OFFEN** (als offen geführt) |

**Zur Bandbreite von `CR-09` ausdrücklich:** Der Vor-W1 geltenden Korrektur folgend wird **nicht** behauptet, dass eine **selektive Row-Teilpropagation** existiert oder dass ein solcher Zustand in W2 beobachtet wurde. Die W2-Evidenz zu `CR-09` besteht **ausschließlich** aus (i) der belegten fehlenden Atomizität zwischen globaler und abgeleiteter Wahrheit und (ii) dem belegten fehlenden Live-Item-/Orphan-Schutz.

### 9.7 Negativtests des W2-Plans: welcher belegt ist und welcher fehlt

Bezug: Plan §7.4, Liste „Negativtests" (6 Einträge).

| # | Negativtest (Plan §7.4) | Stand in W2 | Trägt |
|---|---|---|---|
| 1 | Zwei unabhängige DB-Verbindungen mit konkurrierenden Workflow-Transitions: exakt ein Erfolg, der andere 409, keine ungültige History-Kante. | **belegt** | `backend/workflow/tests/test_transition_concurrency_cr08.py`; Nicht-Vakuizität per Lock-Read-Entfernung belegt ([9.4](#94-aufgedeckte-diskrepanzen-und-fehlversuche) g) |
| 2 | Zwei konkurrierende Single-Interviews: exakt ein Abschluss und eine Audit-/Outbox-Korrelation. | **belegt** für „exakt ein Abschluss"; die **Korrelation im Sinne eines Korrelationsfeldes ist nicht erfüllt** (siehe [9.8](#98-exit-kriterien-des-w2-plans-erfüllt--teilweise--nicht-erfüllt)) | `backend/application/tests/test_interview_formalize_single_race.py`; `django_db(transaction=True)` gesetzt |
| 3 | REST- und MCP-Multi-Interview erzeugen denselben fachlichen Vertrag; fehlendes Proposal ergibt kontrollierten Validierungsfehler. | **belegt**, jedoch **MCP-seitig gegen den vorbestehenden REST-Test** — **nicht** durch eine gemeinsam erzeugte Response | `backend/mcp_server/tests/test_interview_tool_group_formalize_parity.py` |
| 4 | Fehlerinjektion in globaler Propagation hinterlässt weder globales noch abgeleitetes Teilergebnis. | **belegt** | `backend/workflow/tests/test_global_definition_store_atomicity.py`; Nicht-Vakuizität per `atomic()`-Entfernung aus `_persist` belegt |
| 5 | State-Löschung mit Live-Item in nicht individualisiertem Workspace wird abgewiesen. | **belegt** | `backend/rest_api/tests/test_global_default_orphaned_state_409.py` |
| 6 | **Chat-/Provider-Ausfall darf keinen scheinbar vollständigen synchronen Interviewabschluss vortäuschen.** | **FEHLT VOLLSTÄNDIG** | kein Test, keine Implementierung |

**Zu Negativtest 6 im Einzelnen:** **Arbeitspaket 8** (Trennung der **asynchronen** Provider-/Chat-Grenze von der **synchronen** Kerntransaktion) ist **nicht angefasst**. Vorhandene **Chat-Turn**-Tests decken den **Turn** ab, **nicht** den **Formalize-Abschluss**. Der fehlende Negativtest ist damit **kein** Testabdeckungsdetail, sondern eine **nicht begonnene Arbeitseinheit** aus dem Plan.

**Review-Randnotiz zum Test 5:** Die dort zunächst **vermutete** Variante (Duplikat über `(tenant_id, workspace_id, item_type)`) war wegen der bereits existierenden Unique-Constraint `uq_wedef_tenant_ws_type` **nicht erreichbar** und ist durch eine Regressionserkennung mit `IntegrityError` aufgefallen; umgesetzt und belegt ist die erreichbare Variante (Preset-Abweichung einer Zeile), siehe [9.4](#94-aufgedeckte-diskrepanzen-und-fehlversuche) f.

### 9.8 Exit-Kriterien des W2-Plans: erfüllt / teilweise / nicht erfüllt

Bezug: Plan §7.4, Liste „Exit-Kriterien" (5 Einträge). Maßgeblich ist **ausschließlich**, was im Testscope belegt wurde.

| # | Exit-Kriterium | Einstufung | Begründung |
|---|---|---|---|
| 1 | Konkurrierende Writes sind **deterministisch und auditierbar**. | **erfüllt im Testscope** | Zwei-Verbindungen-Negativtest: exakt ein Erfolg, der andere 409, History enthält nur die tatsächlich akzeptierte Kante. „Im Testscope" heißt: ohne DB-Constraint und ohne Revision. |
| 2 | **Keine fehlerbedingte globale/derived Divergenz im Testscope.** | **erfüllt im Testscope, mit dokumentiertem Restfenster** | Fault-Injection belegt kein Teilergebnis. Das **Restfenster** ist bewusst offengelegt und **nicht** geschlossen (siehe [9.9](#99-negativraum-was-ausdrücklich-nicht-geschlossen-ist)). |
| 3 | **REST, MCP und UI erhalten dieselbe versionierte Fehler-/Status-Semantik.** | **NICHT ERFÜLLT** | **REST-409 ist gemessen.** MCP bildet auf **`VALIDATION_ERROR`** ab — konsistent zur bestehenden Konvention in **beiden** Layern. Es existiert **keine versionierte Semantik**, und es wurde **keine einzige Frontend-Datei** geändert. Das Kriterium nennt **drei** Schichten (REST, MCP, **UI**); die UI-Schicht ist unberührt. |
| 4 | **Jede fachliche Mutation hat Tenant, Workspace, Actor, Version und Korrelation.** | **NICHT ERFÜLLT** | **`correlation_id` hat im gesamten Backend 0 Treffer** (verifiziert: `git grep -c "correlation_id" -- backend/` → kein Treffer). **`DomainEventOutbox` führt kein Korrelationsfeld.** Ein Kriterium mit fünf Bestandteilen ist mit einem fehlenden Bestandteil **nicht** erfüllt. |
| 5 | `CR-06`, `CR-07`, `CR-08`, `CR-09`, `CR-13` und die **Multi-Interview-Teilmenge** von `CR-05` sind **entweder verifiziert geschlossen oder explizit als offen ausgewiesen**. | **TEILWEISE ERFÜLLT** | Der **zweite** Ast des Kriteriums ist erfüllt: alle sechs sind in [9.6](#96-was-je-track-tatsächlich-belegt-wurde) **explizit als offen ausgewiesen** — einschließlich `CR-13`, das weder geschlossen noch ausgewiesen ist, sondern als **offen** geführt wird. Der **erste** Ast („verifiziert geschlossen") ist für **keinen** der sechs erreicht, weil keine Revision existiert (siehe [9.1](#91-revision-branch-und-commitbindung)). |

**§11.3 — kein Abbruchkriterium eingetreten.** Keines der Kriterien in Plan §11.3 (globaler Release-Stopp bzw. Wellenabbruch/Neustart) wurde ausgelöst. **Die Welle ist damit nicht gestoppt, aber auch nicht abgeschlossen.** Die W2-Arbeit ist damit weder als Erfolg des Wellenabschlusses noch als abgebrochene Welle zu führen; sie ist ein **zum Schreibzeitpunkt noch uncommitteter, teilweise belegter Zwischenstand** — die festgelegte Commit-Struktur steht in [9.1](#91-revision-branch-und-commitbindung), die abschließende Statuszuschreibung erfolgt in der datierten Provenienz-Auflösung.

### 9.9 Negativraum: was ausdrücklich NICHT geschlossen ist

Jeder Punkt ist **offen** und wird sichtbar weitergereicht.

1. **W2-Negativtest Nr. 6 fehlt vollständig** (Chat-/Provider-Ausfall). Arbeitspaket 8 nicht angefasst. Vorhandene Chat-Turn-Tests decken den Turn, nicht den Formalize-Abschluss.
2. **Exit-Kriterium „dieselbe versionierte Fehler-/Status-Semantik" nicht erfüllt.** REST-409 gemessen, MCP auf `VALIDATION_ERROR` abbildend, **keine** versionierte Semantik, **keine** Frontend-Datei geändert.
3. **Exit-Kriterium „Korrelation" nicht erfüllt.** `correlation_id`: **0** Backend-Treffer (verifiziert). `DomainEventOutbox` führt kein Korrelationsfeld.
4. **Restfenster im `CR-09b`-Gate — bewusst nicht geschlossen, sondern dokumentiert.** Nach der Transaktionsbindung und dem `select_for_update` bleibt ein Fenster, in dem ein Item auf einer **anderen** Connection in den zu löschenden State wechseln kann, **nachdem** das Gate gezählt hat — und zwar **nur** für einen Writer, der gegen einen **veralteten In-Process-Definition-Cache** validiert. Eine **echte** Schließung wäre eine **neue fail-closed State-Deklarationsregel auf dem Transition-Write-Pfad für alle Item-Typen** — mit **realem Falsch-Positive-Risiko**, unter anderem dem System-State `outdated`, der **außerhalb jeder Preset-Liste** liegt. **Bewusst nicht umgesetzt**, sondern dokumentiert in `backend/workflow/global_definition_store.py:348–363`.
5. **Service-Wrapper-Lücke — Asymmetrie zum Interview-Constraint.** Die **ADR-, Risk-, Issue-, Change-Request-, Main-Goal-Service** und der **Goal-reactivate-Pfad** reichen `expected_version` **nicht** durch und bleiben insoweit **Last-Writer-Wins**. Im Code dokumentiert in `backend/application/workflow_facade.py:95–102`. **Im Plan nicht als Folgeschritt vermerkt** — die Asymmetrie zum bewusst deferrierten Interview-Constraint ist damit **im Plan nicht adressiert**. (Die betroffenen Services *besitzen* einen `expected_version`-Parameter auf Service-Ebene — es ist die **Durchreichung durch die Facade-Wrapper**, die fehlt.)
6. **`as_domain_event_outbox` ist eine `CR-17`-Tabelle ohne RLS.** W2 erweitert das `EventType`-Enum um ein **drittes** Interview-Lifecycle-Ereignis (`INTERVIEW_ABANDONED`) und **vergrößert damit die Schreibfläche auf einer bekanntermaßen ungeschützten Tabelle** (verifiziert: `as_domain_event_outbox` steht in `RLS_EXEMPT_TABLES`, `backend/persistence/tests/test_rls_coverage.py:111`). Das **ändert den `CR-17`-Status nicht** und **schließt ihn nicht** — es ist eine **bewusste Konsequenz** und gehört als solche in den Nachweis.
7. **Migration `0027_alter_domaineventoutbox_event_type.py` — angewendet, aber nicht gegen Bestand geprüft.** Verifizierter Inhalt: **genau eine** `AlterField`-Operation auf `event_type` (Datei umfasst 18 Zeilen), Abhängigkeit `('application', '0026_comment_notification_rls')`, **kein** `RunPython`, **keine** Löschung, **keine** Datenmigration, **kein** `UniqueConstraint`, **keine** bestehende Migration editiert, Nummer **automatisch** vergeben. Sie war **nötig**, weil das in W2 ergänzte `INTERVIEW_ABANDONED`-EventType sonst `makemigrations --check` rot gelassen hätte. **Angewendet** im Test-DB-Lauf (die breite Regression lief danach grün) — aber **nicht** gegen einen **befüllten Bestand** geprüft.
8. **E501 in der generator-erzeugten Migration bleibt offen** (siehe [9.4](#94-aufgedeckte-diskrepanzen-und-fehlversuche) c) — inklusive der **nicht** aufgelösten Basiszahl-Diskrepanz (b).
9. **`CR-13` ist unberührt** — kein Test, keine Implementierung; Arbeitspaket 6 des Plans nicht angefasst.
10. **Kein Track ist `VERIFIZIERT`.** Wegen der fehlenden Revision (siehe [9.1](#91-revision-branch-und-commitbindung)). Das betrifft **alle** in [9.6](#96-was-je-track-tatsächlich-belegt-wurde) genannten Tracks einschließlich derer, deren Tests grün sind.
11. **Kein Frontend- und kein Release-Nachweis.** Es wurde keine Frontend-Datei geändert; ein `npm test`/Typecheck-Lauf fand nicht statt und wird nicht behauptet.

### 9.10 Nicht nachgemessene Größen und offene Messungen

Diese Punkte sind **offen**, nicht geglättet:

| # | Offene Messung | Stand |
|---|---|---|
| 1 | Differenz **6330 vs. 6299** bei identischer Skip-Zahl (15) und Exit 0 für dieselben vier Apps. | **nicht erklärt**, keine Zahl gewählt |
| 2 | Migrationsbasiszahl **253** (berichteter E501-Kontext) vs. **270** tracked / **254** ohne `__init__.py` / **255** inklusive der untracked `0027` (gemessen). | **nicht reproduziert**, nicht ersetzt |
| 3 | Scan-Grundlage **28** Slice-Dateien vs. **29** verifizierte Working-Tree-Pfade (18 + 11). | **nicht aufgelöst**; Ursache offen |
| 4 | E501-Trägerzahl (**116**) und `noqa`-Zahl (**0**) in Migrationsdateien. | nicht nachgemessen (kein Lint-Lauf bei der Dokumentation) |
| 5 | Sämtliche Testzahlen und Exit-Codes in [9.3](#93-ausgeführte-kommandos-und-ergebnisse). | **übergebene Messwerte**, bei der Dokumentation nicht nachgemessen |
| 6 | Revisions-/Commit-Bindung des W2-Slices. | **aufgehoben (2026-09-26)** — die Bindung **existiert** jetzt: drei Commits, gepusht, **PR #1073** offen. Siehe [9.17](#917-provenienz-nachtrag-w0w1w2-2026-09-26-pr-1073). **Offen bleibt** die Statuszuschreibung je Track. |
| 7 | `docs/REQUIREMENTS.md` und `docs/superpowers/plans/2026-09-24-motivated-improvement-plan.md`. | **Fortschreibung 2026-09-26:** `docs/REQUIREMENTS.md` ist **unverändert nicht angefasst**. Der **Plan** ist inzwischen zweimal geändert worden: (a) im W2-Slice selbst (Commit `14fc05e5`, **3 Zeilen eingefügt / 1 gelöscht** — der §7.4-Hinweis auf den Lock-Teil plus der benannte Constraint-Folgeschritt, also reine Doku zu CR-06) und (b) durch den **datierten Status-Nachtrag** `## 14. Umsetzungsstand (2026-09-26)`, der den Umsetzungsstand von W0–W5 festhält. Beide Änderungen sind **dokumentarisch**; **kein** Anwendungscode und **keine** Testdatei wurden angefasst. Plan §7.4 enthält den Constraint-Folgeschritt weiterhin (verifiziert, Planzeile 598) — er wird im Nachtrag **referenziert, nicht dupliziert**. |

### 9.11 Working-Tree-Inventar des W2-Slices

Verifiziert über `git status --porcelain=v1 -uall`. **18 tracked geänderte + 11 untracked = 29 Pfade.** Diese Liste ist **das verifizierte Inventar**; sie weicht von der für diesen Nachtrag übergebenen Dateiauswahl ab (siehe [9.4](#94-aufgedeckte-diskrepanzen-und-fehlversuche) i).

**18 tracked geänderte Pfade:**

- `CHANGELOG.md` — ein `## [Unreleased]`/`### Changed`-Eintrag zur REST-Verhaltensänderung; **keine** Version erhöht
- `backend/application/goal_service.py`
- `backend/application/interview_service.py`
- `backend/application/models.py`
- `backend/application/workflow_facade.py`
- `backend/mcp_server/tests/test_interview_tool_group.py` — **in der übergebenen Auswahl nicht genannt**
- `backend/mcp_server/tools/goals.py`
- `backend/mcp_server/tools/interview.py`
- `backend/mcp_server/tools/review.py`
- `backend/rest_api/global_default_views.py`
- `backend/rest_api/mixins/workflow_transitions.py`
- `backend/rest_api/tests/test_views.py` — **in der übergebenen Auswahl nicht genannt**
- `backend/workflow/global_definition_store.py`
- `backend/workflow/lifecycle_manager.py`
- `backend/workflow/management/commands/provision_workflow_definitions.py`
- `backend/workflow/services.py`
- `docs/agent-templates/tool-manifest.json` — generiert
- `docs/superpowers/plans/2026-09-24-motivated-improvement-plan.md` — Doku (§7.4)

**11 untracked (neue) Pfade:**

- `backend/application/migrations/0027_alter_domaineventoutbox_event_type.py`
- `backend/application/tests/test_interview_abandon_formalize_serialization_m2.py`
- `backend/application/tests/test_interview_audit_outbox_seam.py`
- `backend/application/tests/test_interview_formalize_single_race.py`
- `backend/mcp_server/tests/test_interview_tool_group_formalize_parity.py`
- `backend/mcp_server/tests/test_transition_version_conflict_cr08.py`
- `backend/rest_api/tests/test_global_default_orphaned_state_409.py`
- `backend/rest_api/tests/test_transition_expected_version_cr08.py`
- `backend/workflow/tests/test_global_definition_store_atomicity.py`
- `backend/workflow/tests/test_provision_workflow_definitions.py`
- `backend/workflow/tests/test_transition_concurrency_cr08.py`

**Geplante Zuordnung der 29 Slice-Pfade zu den drei Commits.** Diese Zuordnung ist **aus dem Dateiinhalt abgeleitet** — verifiziert per Grep über die Working-Tree-Diffs auf drei Markerfamilien: `expected_version` (Commit 1), `INTERVIEW_ABANDONED` / `formalize` / `_lock_in_progress_session` (Commit 2), `would-orphan-live-item` / `select_for_update` / `transaction.atomic` (Commit 3) — und **nicht** aus einer Git-Historie, weil noch **kein** W2-Commit existiert. Sie ist damit eine **Planungsangabe** zur Erleichterung der späteren Provenienzprüfung und wird je Commit über `git show --stat <sha>` verifiziert. Summe: **12 + 10 + 6 = 28** Slice-Pfade plus **1** generiertes Manifest = **29**.

| # | Commit-Message (exakt, Conventional Commits) | Pfade | Dateibezug (verifiziert) |
|---|---|---|---|
| 1 | `fix(CR-08): enforce expected_version through the workflow lock` | **12** | `CHANGELOG.md` (der Diff ist **ausschließlich** ein `## [Unreleased]`/`### Changed`-Eintrag zu `expected_version`/`If-Match`/409, also rein CR-08); `backend/application/goal_service.py`; `backend/application/workflow_facade.py`; `backend/mcp_server/tools/goals.py`; `backend/mcp_server/tools/review.py`; `backend/rest_api/mixins/workflow_transitions.py`; `backend/rest_api/tests/test_views.py`; `backend/workflow/lifecycle_manager.py`; `backend/workflow/services.py`; `backend/mcp_server/tests/test_transition_version_conflict_cr08.py`; `backend/rest_api/tests/test_transition_expected_version_cr08.py`; `backend/workflow/tests/test_transition_concurrency_cr08.py` |
| 2 | `fix(CR-06/CR-07/CR-05): serialise and audit interview formalization` | **10** | `backend/application/interview_service.py` (Row-Lock auf der Interview-Session, Audit im selben `atomic()`-Block); `backend/application/models.py` (`INTERVIEW_ABANDONED`); `backend/application/migrations/0027_alter_domaineventoutbox_event_type.py`; `backend/application/tests/test_interview_abandon_formalize_serialization_m2.py`; `backend/application/tests/test_interview_audit_outbox_seam.py`; `backend/application/tests/test_interview_formalize_single_race.py`; `backend/mcp_server/tests/test_interview_tool_group.py`; `backend/mcp_server/tools/interview.py` (`confirmed_proposal`); `backend/mcp_server/tests/test_interview_tool_group_formalize_parity.py`; `docs/superpowers/plans/2026-09-24-motivated-improvement-plan.md` (der Diff ist **ausschließlich** der §7.4-Hinweis auf den Lock-Teil plus der benannte Constraint-Folgeschritt zu Arbeitspaket 2, also rein CR-06) |
| 3 | `fix(CR-09/CR-10): make global definition propagation atomic and orphan-safe` | **6** | `backend/rest_api/global_default_views.py` (`OrphanedStateError` → 409, CR-09b); `backend/workflow/global_definition_store.py`; `backend/workflow/management/commands/provision_workflow_definitions.py`; `backend/rest_api/tests/test_global_default_orphaned_state_409.py`; `backend/workflow/tests/test_global_definition_store_atomicity.py`; `backend/workflow/tests/test_provision_workflow_definitions.py` |
| — | **generiertes Artefakt**, siehe 9.12 — **nicht** in Commit 1 | **1** | `docs/agent-templates/tool-manifest.json` |

**Zuordnung, die bewusst offen bleibt:** `docs/agent-templates/tool-manifest.json` trägt Schemas aus **zwei** Commits (Commit 1: **3** neue `expected_version`-Einträge; Commit 2: **1** neuer `confirmed_proposal`-Eintrag) und kann deshalb **nicht** in Commit 1 liegen — Begründung und die daraus folgende **begründete Zwischenzustands-Abweichung** stehen in 9.12. Commit 3 enthält **keine** MCP-Tool-Schemaänderung (keiner seiner sechs Pfade liegt unter `backend/mcp_server/tools/`), weshalb das Manifest in Commit 2 **oder** — bei Regeneration ganz am Ende — in Commit 3 aufgenommen werden kann. Die **exakte Platzierung ist eine Entscheidung des Git-Durchgangs**, keine Tatsache dieses Registers. **Dieses Evidenzregister selbst** (19. tracked geänderter Pfad) ist **kein** W2-Slice-Pfad und in der Zuordnungstabelle deshalb **nicht** enthalten.

### 9.12 Generiertes Manifest `docs/agent-templates/tool-manifest.json` — einschließlich begründeter Zwischenzustands-Abweichung

`docs/agent-templates/tool-manifest.json` ist ein **generiertes** Artefakt, kein Handgeschriebenes. Verifiziert über den Working-Tree-Diff: die Regeneration fügt **genau vier** Schema-Blöcke hinzu — **drei** `expected_version`-Einträge (Commit 1, aus `backend/mcp_server/tools/goals.py:301` und `backend/mcp_server/tools/review.py:87,133`) und **einen** `confirmed_proposal`-Eintrag (Commit 2, aus `backend/mcp_server/tools/interview.py:211`, Tool `interview.formalize`).

**Konsequenz — eine bewusst in Kauf genommene Zwischenzustands-Abweichung.** Wegen dieser Herkunft aus **zwei** Commits wird das Manifest **einmal am Ende** regeneriert und **nicht** in Commit 1 aufgenommen. Der Drift-Guard `backend/mcp_server/tests/test_tool_manifest_drift.py::test_committed_manifest_matches_live_registry` vergleicht das committete Manifest **bidirektional** mit der Live-Registry und prüft dabei ausdrücklich `is_write`, `prefix`, `description` und `inputSchema` (verifiziert per Code-Lesung, Zeilen 108–146). Daraus folgt:

| Commit-Zustand | Erwartung für `test_committed_manifest_matches_live_registry` | Herkunft der Erwartung |
|---|---|---|
| Commit 1 (Manifest **ohne** Commit 1) | **rot** — `inputSchema`-Drift auf den **drei** `expected_version`-Tools, weil die Registry den Commit-1-Stand schon trägt, das committete Manifest nicht | abgeleitet aus dem Guard-Code und der Manifest-Herkunft, **nicht** gemessen |
| Commit 2 (Manifest mit Commit 1 **und** 2) | **grün** | dito |
| Commit 3 | **grün**, weil Commit 3 keine MCP-Tool-Schemaänderung enthält | dito |

**Ausdrücklich festgehalten:** Dies ist eine **Begründung, kein Messwert.** Bei dieser Dokumentation wurde **kein** Test ausgeführt; die Rot-/Grün-Aussage ist eine **abgeleitete Erwartung** aus dem Guard-Code, keine beobachtete Ausführung. Wird das Manifest abweichend in **Commit 3** aufgenommen, verschiebt sich die grüne Stufe um einen Commit (Commit 1 **und** 2 rot, Commit 3 grün) — der **Endzustand** ist in beiden Fällen identisch. Der Zwischenzustand ist damit **dokumentiert und begründet**, nicht verdeckt; die in [9.3](#93-ausgeführte-kommandos-und-ergebnisse) genannte grüne Manifest-Suite („**5 passed**") bezieht sich auf den **W2-Endzustand** des Working Trees, also auf den Stand **mit** regeneriertem Manifest, und **nicht** auf einen der Commits 1–3.

### 9.13 Geplanter Pull Request

| Feld | Festgehaltener Stand |
|---|---|
| Titel | `fix: enforce W2 atomicity and concurrency contracts` |
| Ziel-Branch | `main` |
| Quelle | `feat/audit-w2-atomicity-concurrency` (Basis `acde772e`, drei Commits aus [9.1](#91-revision-branch-und-commitbindung)) |
| Maßgebliche Zuordnung | **Der W2-Revisionsstand im PR ist die maßgebliche Zuordnung** dieses Nachweises: die Zuordnung eines Tracks zu einer Revision erfolgt über den PR-Stand, nicht über den flüchtigen Working Tree. Die abschließende, datierte Statuszuschreibung folgt in der Provenienz-Auflösung. |
| PR-Nummer / Merge-SHA | **existieren zum Schreibzeitpunkt nicht** und werden hier **nicht** vorweggenommen. Sie entstehen erst durch den nachfolgenden Git-Durchgang; die Auflösung folgt dem Muster aus [7.4](#74-auflösung-der-provenienz-2026-09-25) und 7.5. |
| **Tatsächlicher PR-Stand** (Fortschreibung 2026-09-26) | **PR-Nummer: `#1073`** — **offen** gegen `main`, Quelle `feat/audit-w2-atomicity-concurrency`, Titel `fix: enforce W2 atomicity and concurrency contracts` (verifiziert per `gh pr view 1073`). **Ein Merge-SHA existiert nicht**, weil der PR **nicht gemergt** ist; es wird hier **keiner** vorweggenommen. Die Zuordnung dieses Nachweises ist damit über die **PR-Revision** hergestellt, und die vollständige Kette steht in [9.17](#917-provenienz-nachtrag-w0w1w2-2026-09-26-pr-1073). |

### 9.14 Deferred D1-Folgeschritt (rein deskriptiv)

**Weder terminiert noch mit eigener Nummer versehen.** Wird hier ausschließlich beschrieben, nicht behauptet, nicht terminiert.

- **Geplant:** `UniqueConstraint(fields=["session", "artifact"])` auf `InterviewSessionArtifact` — Tabelle **`pl_interview_session_artifact`** (verifiziert: `db_table` in `backend/persistence/models.py:2918`), App **`persistence`**.
- **Expand-only**, weil die Spalten bereits existieren: kein Daten-Umbau, kein Backfill, kein Contract-Schritt.
- Der Constraint gilt dem **Paar** und **nicht** `session` allein, weil **Multi-Sessions legitim eine Zeile je Artefakt** anlegen.
- **Vor der Migration ist ein Duplikat-Vorcheck** über bestehende Zeilen **nötig** (Gruppierung nach `session_id, artifact_id`, Null-Gruppen mit `count(*) > 1` als Erwartungswert), weil die Tabelle **älter als das Locking** ist und die Constraint sonst erst zur Apply-Zeit scheitern würde.
- **Grund des Deferments:** eine gesonderte **Migrationsfreigabe** fehlte in diesem Durchgang.
- **Verwandte Code-Stelle:** `backend/application/interview_service.py`, Docstring von `_lock_in_progress_session`.

### 9.15 W1-Grenze: Bestätigung der unverändert offenen W1-Restpunkte

Dieser Nachtrag **ändert keinen** W1-Befund. Die folgenden W1-Restpunkte aus [8.6](#86-verbleibendes-restrisiko-bewusst-offen-geführt), [8.6.1](#861-priorisierte-nacharbeiten-aus-der-review-runde-zu-pr-1071-2026-09-25) und [8.5](#85-w1-exit-kriterien-nur-tatsächlich-verifiziert-grün) sind durch W2 **nicht** geschlossen worden:

| W1-Restpunkt | Bestätigung |
|---|---|
| **`CR-17`** — die vier worker-eigenen Plain-Tabellen `as_domain_event_outbox`, `as_domain_event_dlq`, `as_webhook_subscription`, `as_webhook_delivery_log` (keine RLS, keine `tenant_id`-Spalte, keine ausdrückbare Policy; Django-Admin-Superuser als menschlich erreichbarer Cross-Tenant-Pfad) | **unverändert offen.** W2 **vergrößert** die Schreibfläche auf `as_domain_event_outbox` sogar (siehe [9.9](#99-negativraum-was-ausdrücklich-nicht-geschlossen-ist) Punkt 6) — **ohne** den Track-Status zu ändern oder ihn zu schließen. |
| **`CR-22`** — Batch-Fehlervertrag und Hermes-TypeScript-Timeout-/Retry-Verträge; `X-Project-ID` und `X-User-ID` nicht eigenständig getestet | **unverändert offen.** W2 hat **keinen** dieser Punkte berührt. |
| **`CR-26`** — Revocation der Refresh-Familie nach Passwortwechsel (kein Endpunkt, kein Revocation-Hook) | **unverändert offen.** W2 hat diesen Punkt nicht berührt. |
| **`pl_user`** — Pre-Authentication-Lookup auf einer Tabelle ohne RLS (P1 aus der Review-Runde zu PR #1071) | **unverändert offen.** W2 hat `pl_user` **nicht** angefasst — weder gepatcht noch wegkonfiguriert noch als akzeptiert markiert. |

**Über diese Grenze hinaus gilt:** Die in [8.7.4](#874-was-diese-review-runde-nicht-geändert-hat) genannten W1-Nacharbeiten (`pl_user`, `X-Project-ID`/`X-User-ID`, die drei ungelisteten Testverzeichnisse `backend/link_types/tests/`, `backend/memory/tests/`, `backend/tests/`) sind in W2 **nicht** bearbeitet worden. Der in [8.6](#86-verbleibendes-restrisiko-bewusst-offen-geführt) Punkt 8 — der CI-Job `workflow-lint` ist noch nicht auf einem echten CI-Runner gelaufen — bleibt ebenfalls **unberührt**; W2 liefert **keinen** CI-Lauf.

### 9.16 Was W2 ausdrücklich nicht getan hat

- **Kein Track-Status kehrt sich.** Alle Statuszellen in [Abschnitt 2](#2-kanonisches-finding-trackregister) bleiben unverändert.
- **Kein Track wurde als `VERIFIZIERT` markiert.**
- **Keine Zahl wurde durch eine andere ersetzt** — abweichende Größen stehen unglättiert nebeneinander ([9.4](#94-aufgedeckte-diskrepanzen-und-fehlversuche) a–c, [9.10](#910-nicht-nachgemessene-größen-und-offene-messungen)).
- **Keine erfundenen REQ-, Issue- oder CR-IDs.** Genannt werden ausschließlich die im Register bereits bestehenden `CR-*`-Labels und existierende Artefakte mit Pfad.
- **`docs/REQUIREMENTS.md` nicht angefasst.**
- **`docs/superpowers/plans/2026-09-24-motivated-improvement-plan.md` inhaltlich nicht angefasst** — die Datei erscheint im Working Tree als geändert; diese Änderung stammt aus dem Slice, nicht aus diesem Nachtrag. Plan §7.4 enthält den Constraint-Folgeschritt bereits. **Fortschreibung 2026-09-26:** Der Plan trägt zusätzlich den datierten Statusabschnitt `## 14. Umsetzungsstand (2026-09-26)`; die Abschnitte 0–13 und der `Abschluss`-Block bleiben **unverändert**. Die Wellenabschnitte liegen in §7.2–§7.7, die Abbruchkriterien in §11.3.
- **Kein Commit, kein Push, kein PR, kein Branch-Wechsel** bei der Erstellung dieses Nachtrags — der Arbeitsbranch `feat/audit-w2-atomicity-concurrency` wurde nicht verlassen. Die **festgelegte** Commit- und PR-Struktur des nachfolgenden Git-Durchgangs steht in [9.1](#91-revision-branch-und-commitbindung) und 9.13; sie ist eine **Planungsangabe**, keine in diesem Register vollzogene Handlung. **Fortschreibung 2026-09-26:** Dieser Durchgang ist **danach** erfolgt — drei Commits, Push und **PR #1073** (offen). Die Aufzählung oben bleibt als Aussage über den **Schreibzeitpunkt** dieses Nachtrags erhalten und ist **kein** Widerspruch zur späteren Handlung.
- **Keine SHA erfunden.** Genannt werden ausschließlich die **verifizierten** Revisionen `acde772e…` (Basis), `59bcb7a9…` (Merge PR #1070), `82f13395…` und `f85407f7…` (W1-Kette aus [7.4](#74-auflösung-der-provenienz-2026-09-25)/7.5) sowie `22aeae4e` (Abgrenzung, siehe [9.1](#91-revision-branch-und-commitbindung)). Für die drei W2-Commits und den Merge-/PR-Commit wird **keine** SHA genannt, weil **keine existiert**. **Fortschreibung 2026-09-26:** Für die drei W2-Commits werden jetzt die **real existierenden, verifizierten** SHAs genannt (`6f145c87…`, `14fc05e5…`, `d4d912bf…`). Es wird weiterhin **keine** SHA **erfunden**, und für den **Merge** von PR #1073 wird **keine** SHA genannt, weil der PR **offen** ist und ein Merge-SHA daher **nicht existiert**.
- **Kein Test, kein Lint, kein Migrationstest wurde bei der Erstellung dieses Nachtrags ausgeführt.** Die Verifikation dieses Abschnitts beschränkte sich auf lesende Git- und grep-Kommandos.

### 9.17 Provenienz-Nachtrag W0/W1/W2 (2026-09-26, PR #1073)

Dieser Abschnitt ist ein **dritter datierter Nachtrag** zum Wellenstand, nach dem Muster aus [7.4](#74-auflösung-der-provenienz-2026-09-25) und [7.5](#75-weiterführung-der-provenienz-auflösung-2026-09-25-pr-1071). Er hebt die **letzte** in [9.1](#91-revision-branch-und-commitbindung) verbliebene offene Revisionsaussage auf.

**Abgrenzung — was dieser Nachtrag nicht verändert:**

- Die Abschnitte **1–8** bleiben **inhaltlich unverändert**; insbesondere bleiben die **Statuszellen der kanonischen Tracks** in [Abschnitt 2](#2-kanonisches-finding-trackregister), die Widerspruchstabelle in [4](#4-widersprüche-und-offene-reconciliation-entscheidungen) und die Abschlussregel in [§6](#6-pflege--und-abschlussregel) unangetastet.
- Die **gemessenen Evidenzzeilen** der Abschnitte 9.2–9.9 sowie 9.11 und 9.12 (gemessene Gates, Diskrepanzen, Working-Tree-Inventar, Manifest-Begründung) bleiben **unverändert**. Fortgeschrieben wurden ausschließlich die **Revisionszeilen** in [9.1](#91-revision-branch-und-commitbindung), die **zwei Revisions-/Dateiaussagen** in der Offene-Messungen-Tabelle [9.10](#910-nicht-nachgemessene-größen-und-offene-messungen) (Zeilen 6 und 7), der **PR-Stand** in [9.13](#913-geplanter-pull-request) sowie die **historischen Bezugsaussagen** in [9.16](#916-was-w2-ausdrücklich-nicht-getan-hat) — jeweils mit sichtbarer Datumsmarke. Die **übrigen** Zeilen von [9.10](#910-nicht-nachgemessene-größen-und-offene-messungen) bleiben als offene Messungen unverändert.
- **Keine Zahl wird durch eine andere ersetzt.** Die abweichenden Größen `6330` / `6299` und `253` / `270` bleiben unglättiert nebeneinander stehen.

**Vollständige Provenienzkette der Wellen (Stand 2026-09-26):**

| Welle | Feld | Festgehaltener Stand |
|---|---|---|
| **W0** — Evidence-Baseline | Umsetzungsstand | **teilweise umgesetzt.** Die **W0-Exit-Kriterien sind nicht vollständig erfüllt.** |
| **W0/W1-Slice** | Commit | `82f1339595485e4706702bff5ebe8a5760d22be4` — „fix: harden W0/W1 security boundaries" |
| **W0/W1-Slice** | PR / Merge | PR **#1070**, **gemergt** als `59bcb7a91e17b13cfedd1476073a2d1b6e8d040d` — „Merge pull request #1070 from Popoboxxo/feat/audit-w0-w1-security" |
| **W1** — Security und Tenant-Isolation | Umsetzungsstand | **Security-Slice und Close-out gemergt.** Die **W1-Exit-Kriterien sind nicht vollständig erfüllt**; die W1-Restpunkte bleiben unverändert offen. |
| **W1-Close-out** | Commit | `f85407f7b9684426dec989ade265d1bc4eb70217` — „fix: close W1 security audit gaps" |
| **W1-Close-out** | PR / Merge | PR **#1071**, **gemergt** als `acde772e7def60dfe70b1ee0188ce28f152d7ee5` — „Merge pull request #1071 from Popoboxxo/feat/audit-w1-closeout"; darin enthalten `7012a7c2c1db9662321621df59ad31160998b2dc` — „fix: address PR 1071 review findings" |
| **W2** — Workflow, Atomizität, Konsistenzkern | Umsetzungsstand | **in drei Commits umgesetzt; PR #1073 offen.** Die **W2-Exit-Kriterien sind nicht vollständig erfüllt** → Status **`TEILWEISE VERIFIZIERT`**. |
| **W2** | Commit 1 | `6f145c87428971406c62c0d69793a65f7a1feab3` — „fix(CR-08): enforce expected_version through the workflow lock" |
| **W2** | Commit 2 | `14fc05e58f15eb567e06403c3f3efd0a474ae2fe` — „fix(CR-06/CR-07/CR-05): serialise and audit interview formalization" |
| **W2** | Commit 3 | `d4d912bf595e821820ea6989342e272e888e9a3a` — „fix(CR-09/CR-10): make global definition propagation atomic and orphan-safe" |
| **W2** | Branch | `feat/audit-w2-atomicity-concurrency`, **gepusht** (`origin/feat/audit-w2-atomicity-concurrency` = `d4d912bf…`) |
| **W2** | PR | **#1073** — **offen** gegen `main`, Titel „fix: enforce W2 atomicity and concurrency contracts". **Ein Merge-SHA existiert nicht.** |
| **W2** | Basis | `acde772e` (= `origin/main`); Diff **30 Dateien, +4858 / −153** |
| **W3**, **W4**, **W5** | Umsetzungsstand | **nicht begonnen.** Keine Commits, keine PR, keine Revision. |

**Verifikationsgrundlage dieses Nachtrags** (ausschließlich lesende Kommandos, keine Git-Mutation):

| Kommando | Ergebnis |
|---|---|
| `git log -1 <sha>` (je SHA) | Betreff und Hash der acht genannten Revisionen bestätigt |
| `git rev-parse origin/main` | `acde772e7def60dfe70b1ee0188ce28f152d7ee5` |
| `git rev-parse origin/feat/audit-w2-atomicity-concurrency` | `d4d912bf595e821820ea6989342e272e888e9a3a` — identisch mit `HEAD`, damit **gepusht** |
| `git diff --shortstat acde772e HEAD` | `30 files changed, 4858 insertions(+), 153 deletions(-)` |
| `git status --porcelain` | **leer** — Slice vollständig committet |
| `gh pr view 1070 / 1071 / 1072 / 1073 --json …` | **#1070** MERGED (`59bcb7a9`), **#1071** MERGED (`acde772e`), **#1072** OPEN (`chore/refresh-project-metadata`), **#1073** OPEN (`feat/audit-w2-atomicity-concurrency`, base `main`) |

**Was aus der Revisionsbindung folgt — und was ausdrücklich nicht folgt:**

- **W2 ist committet, gepusht und revisionsgebunden.** Die Zuordnung eines Tracks zu einer Revision ist damit nicht mehr flüchtig.
- **W2 bleibt `TEILWEISE VERIFIZIERT`.** Die Revisionsbindung ist **nur eine** der Bedingungen aus [§6](#6-pflege--und-abschlussregel). Weil die Exit-Kriterien **nicht vollständig** erfüllt sind ([9.8](#98-exit-kriterien-des-w2-plans-erfüllt--teilweise--nicht-erfüllt)), wird **kein** Track als `VERIFIZIERT` markiert. **Grüne Tests allein begründen keinen `VERIFIZIERT`-Status.**
- **Kein Wellenabbruch.** Kein Kriterium aus §11.3 des Plans (Abbruch- und Stop-Kriterien) wurde ausgelöst; W2 ist **nicht gestoppt, aber auch nicht abgeschlossen.**
- **Keine Statuszelle** in [Abschnitt 2](#2-kanonisches-finding-trackregister) wurde umgeschrieben. Die **Statusschreibung der einzelnen W2-Tracks** ist ein **eigener, datierter Nachtrag** und steht zum Schreibzeitpunkt **aus**.
- **Die Commit-Struktur wurde eins zu eins umgesetzt** — Betreffe und Reihenfolge der drei Commits stimmen mit der in [9.11](#911-working-tree-inventar-des-w2-slices) geplanten Zuordnung überein.
- **Die Manifest-Zwischenzustands-Ableitung bleibt eine Ableitung, kein Messwert.** `mcp_server/tests/test_tool_manifest_drift.py::test_committed_manifest_matches_live_registry` ist im Zustand von **Commit 1** strukturell **rot**, weil das generierte Manifest Schemas aus **Commit 1 und Commit 2** enthält und erst mit Commit 2 regeneriert wurde. Das ist eine **aus dem Guard-Code und der Manifest-Herkunft abgeleitete Erwartung**, **kein** gemessener Testlauf — Herleitung und Grenzen vollständig in [9.12](#912-generiertes-manifest-docsagent-templatestool-manifestjson--einschließlich-begründeter-zwischenzustands-abweichung).
- **Getrenntes Vorhaben, kein W2-Artefakt:** **PR #1072**, Branch `chore/refresh-project-metadata`, Commit `22aeae4e` („chore: refresh project metadata"), betrifft ausschließlich `.meta-config/project.yaml`, **offen** gegen `main`. Inhalte oder Werte daraus werden **nicht** in dieses Register übernommen.

**Offene Restpunkte nach Provenienzauflösung** (alle **offen**, sichtbar weitergereicht):

| Restpunkt | Stand |
|---|---|
| D1-Constraint `UniqueConstraint(session, artifact)` | **bewusst zurückgestellt**, vollständig beschrieben in [9.14](#914-deferred-d1-folgeschritt-rein-deskriptiv) und im Plan §7.4 |
...
| `CR-13` | **unberührt** |
| `CR-17`, `CR-22`, `CR-26` | **unverändert offen** — siehe [9.15](#915-w1-grenze-bestätigung-der-unverändert-offenen-w1-restpunkte) |
| `pl_user` | **unverändert offen** |
| Service-Wrapper-`expected_version`-Lücke | **offen** |
| Orphan-Gate-Restfenster | **offen, bewusst nicht geschlossen** |
| Differenz `6330` / `6299` (+31) | **offen** — nicht erklärt, erklärbar durch die in der Review-Fix-Runde ergänzten Tests; **keine** Zahl ersetzt |

**Offene Messung zu diesem Nachtrag selbst.** Der **Doku-Commit**, der diesen Nachtrag sowie den Statusabschnitt im Plan aufnimmt, hat **zum Schreibzeitpunkt noch keine SHA**. Sie wird hier **nicht** vorweggenommen. Die Revision dieses Nachtrags ist ausschließlich über **`feat/audit-w2-atomicity-concurrency` + die drei W2-SHAs (`6f145c87…`, `14fc05e5…`, `d4d912bf…`) + PR #1073** bestimmt. **Die exakte SHA des Doku-Commits ist eine offene Messung**, die ein **späterer, datierter Nachtrag** nachträgt — nach dem Muster aus [7.4](#74-auflösung-der-provenienz-2026-09-25) und [7.5](#75-weiterführung-der-provenienz-auflösung-2026-09-25-pr-1071).

**Statusnotiz zu `deploy/.env`:** Die Datei wurde für diesen Nachtrag **weder verändert noch gelesen**; es werden **keine** Werte aus ihr wiedergegeben.

**Platzierung:** Dieser Nachtrag steht **vor** dem terminalen `STATUS:`-Block am Dateiende. Dieser Block bleibt der **historische Abschlussvermerk des Audits vom 2026-09-24** und wird weder ergänzt noch umformuliert; er ist **nicht** der W2-Abschlussvermerk.
