...
from diagram.services import create_diagram
...
def run():
    ...
    ctx = AuthContext(
        tenant_id=tenant.id, 
        user_id=user.id, 
        active_roles=("tenant_admin",),
        auth_method=AuthMethod.BEARER_TOKEN
    )
    
    ws_svc = WorkspaceService()
    print("Creating Workspace...")
    ws = ws_svc.create_workspace(ctx=ctx, name="Zahnbürste SysEng Demo", preset="extended", terminology_profile="se_mode")
    ws_id = ws.id
