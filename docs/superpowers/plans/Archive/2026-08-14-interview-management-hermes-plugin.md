...
- **`interview.set_target` (issue #540, MCP tool merged 2026-08-15) postdates this plan** and is not consumed by any task here — it was added to the backend one day after this plan's original write. Not a gap in this plan's own scope (this plan predates that tool's existence), but a natural candidate for a small follow-up task once this plan is implemented: letting `InterviewFormView` call it when the user confirms a grounding-candidate match, the same way the Hermes-plugin design's grounding-hint display already anticipates.
