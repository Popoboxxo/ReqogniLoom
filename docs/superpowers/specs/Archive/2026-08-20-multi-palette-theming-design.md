...
| 3 | ~~Paletten-Freischaltung + WCAG-Kontrast-Tests~~ — **erledigt** | siehe `docs/superpowers/plans/2026-08-21-multi-palette-theming-phase3.md`, 4 Tasks + 1 Controller-Fix (fehlende Shadow/Gradient/Glass-Tokens), alle Reviews clean. 3 neue Themes: `bauhaus` (User-Spec, exakte Anchor-Hex), `nordic` (Nord-inspiriert, 2 Primary-Shades für WCAG AA angepasst), `sepia` (warmes Editorial-Theme, 1 Primary-Shade angepasst). Neuer WCAG-Kontrast-Test (`theme-contrast.test.ts`, 35 Assertions über alle 5 Themes) deckt dabei einen vorbestehenden, unabhängigen Bug im `dark`-Theme auf (`on-primary`/`primary` = 4.47:1, verfehlt AA um 0.03) — bewusst nicht in diesem Scope gefixt, als `KNOWN_AA_SHORTFALLS`-Eintrag dokumentiert, eigenes Follow-up nötig. 2280/2281 Backend + 1142/1143 Frontend grün (bekannte, unabhängige Vorbestandsfehler) |
