...
- Name: mermaid-diagram.spec.ts >> [REQ-L1-057 / REQ-L2-DS-007] Mermaid Editor >> [REQ-L1-057] test_mermaid_valid_code_renders_preview
- Location: tests\mermaid-diagram.spec.ts:110:7
...
> 19  |   await page.click('button[type="submit"]');
  ...
  116 |   if (!response.ok()) {
  117 |     throw new Error(
  118 |       `Failed to set workspace preset to '${preset}': ${response.status()} ${await response.text()}`
  119 |     );
```