...
**Partly superseded, partly re-scoped (2026-09-15) — not implemented.** Sixth of 11. The owner/assignment half was overtaken by the merged Attribut-System v3 (#934/#936/#940), which put attribution on one central `Artifact.owner`/`Artifact.reporter` pair of `Actor` FKs (`backend/persistence/models.py:1096-1111`), and by the AWMS migration plans (`risk_owner_to_actor.yaml`, `issue_assignee_to_actor.yaml`) — tasks 1–6, 12, 13, 14, 21, 26 are cancelled, with the physical column drop left as an AWMS-owned contract step.
