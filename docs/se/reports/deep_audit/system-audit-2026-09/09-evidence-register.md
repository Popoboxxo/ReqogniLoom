...
| Festgelegte Commit-Struktur | **Drei** bounded Commits in dieser Reihenfolge (Conventional Commits, erste Zeile jeweils max. 72 Zeichen, exakt so beabsichtigt):<br>1. `fix(CR-08): enforce expected_version through the workflow lock`<br>2. `fix(CR-06/CR-07/CR-05): serialise and audit interview formalization`<br>3. `fix(CR-09/CR-10): make global definition propagation atomic and orphan-safe` |
...
| Nachprüfbarkeit vor der SHA | Bis die SHAs vorliegen, ist die Zuordnung revisionsgebunden über **Branch + die drei Commit-Messages + die PR-Revision**. Eine SHA wird **nicht** vorweggenommen. |
...
| 6 | Revisions-/Commit-Bindung des W2-Slices. | **existiert nicht** — bewusst offengelassen |
| 7 | `docs/REQUIREMENTS.md` und `docs/superpowers/plans/2026-09-24-motivated-improvement-plan.md`. | **nicht angefasst**; Plan §7.4 enthält den Constraint-Folgeschritt bereits (verifiziert, Planzeile 598) |
