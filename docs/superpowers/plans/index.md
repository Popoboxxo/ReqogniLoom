...
**Amendment (2026-09-15): OD-1…OD-4 resolved, scope extended by five tasks.** OD-1 resolved *with a scope extension*: notification delivery becomes configurable **in the user's own profile** — one **user-global** opt-out preference (`auth_tenancy.UserNotificationPreference`, next free prefix `auth_tenancy/0014` + a mandatory RLS migration `0015`; JSON `disabled_triggers`, not four boolean columns), **four switches** (`assigned`/`comment_added`/`transition_pending`/`suspect_flagged`, all on by default), filtered in exactly **one** place inside `create_notifications` via `NotificationPreferenceService.apply_preferences` — which is why the new Task 27 runs **before** Task 10 despite its higher number. New tasks **26** model+migrations, **27** filter+service, **28** self-service REST `/api/v1/users/me/notification-preferences/` (Layer-2 only, zero direct-ORM lines — the `rest_api/*_views.py` glob ratchet covers it), **29** `NotificationsSection` in `UserProfileSettings`, **30** de/en i18n. OD-2 `Comment.author` = `User` FK; OD-3 `comment_added` fan-out = owner + reporter, author excluded; OD-4 no suspect notification to external owners. Three assumptions recorded (per-`(tenant,user)` scoping, JSON list over boolean columns, filter fails **open**) and one editorial item left open (**OD-5**: the re-scope's Task 26 re-uses a *dropped* original-plan number 26). Out of scope by decision: e-mail/webhooks/digest, per-workspace preferences, notification history. **Still nothing implemented.**
