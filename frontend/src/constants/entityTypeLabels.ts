 ...
 * label artifact entity types (CsvImport).
 *
 * Keys match the backend's PascalCase artifact_type values where the
 * consumer already works in that vocabulary (CsvImport); a consumer that
 * works in a different local vocabulary references the same key constants
