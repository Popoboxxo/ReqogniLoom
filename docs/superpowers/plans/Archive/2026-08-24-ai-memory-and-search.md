...
**Added post-implementation, per the final whole-branch review (2026-08-26) — real gaps, ruled deferred rather than left silent:**

- `scope="tenant"` cross-workspace search (all of Task 8's plumbing) is implemented but unreachable from any real client — neither the REST search view nor the MCP `artifact.search` tool passes a `scope` param through. Follow-up task: thread a validated `scope` param through both surfaces (+ MCP manifest regen). No functional regression from deferring — single-workspace search is unaffected.
- REST equivalents under `/api/v1/memory/` for `query`/`list`/`forget` (spec's own §API/MCP asked for these; only the MCP tool group and the settings REST endpoints exist). MCP-only access is sufficient for v1.
- Semantic search fusion does not cover `WorkspaceMemory`/`UserTenantMemory` embeddings (spec's §Suche asked for this in addition to `Requirement`/`TraceLink`/`IcdVersion`) — a genuine missed value-add, since these are the one embedding source that actually works dimension-wise under the shipped `sentence-transformers` (384-dim) default, unlike the 1536-dim `Requirement`/etc. fields. Follow-up task: a new semantic pass specifically over the two memory models, fused the same way Task 9 does for the others.
- `AiDerivationService` methods never got `build_memory_context()` wiring (spec's §Prompt-Integration named this alongside `InterviewService.generate_chat_turn()`; only the interview single-mode chat-turn path was wired, per Task 6's own confirmed scope). Additive-only follow-up, no regression risk from deferring.
