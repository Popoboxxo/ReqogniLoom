...
**Not implemented — plan written (2026-09-03).** Fourth of 11. 16 tasks, 112 steps. Caught and fixed a real bug the spec would have introduced: `WorkflowDefinitionDTO.initial_state` is `states[0]`, and the spec's JSON put `"proposed"` at position 0 — every artifact, including human-authored ones, would have started as a proposal. **One open question, not blocking:** confirming a link nulls `TraceLink.proposed_by`/`proposed_at` entirely (§5), destroying provenance — unlike artifacts, where the workflow history entry preserves who proposed it. Intentional or a spec inconsistency? Not yet confirmed by the user.

## [2026-09-03-interview-engine-fix.md](2026-09-03-interview-engine-fix.md)
**Not implemented — plan written (2026-09-03).** Fifth of 11. 15 tasks (14 executable, 1 blocked on the datenmodell-konsolidierung plan's GlossaryTerm Artifact-backing). No blocking questions. Four spec gaps closed: Risk still couldn't be formalized after the spec's own fix (missing required fields in the default protocol), `resulting_artifact_ids` would have regressed a different feature (issue #736), the provenance badge would have stayed invisible for ~90% of interviews (single-kind sessions never write `InterviewSessionArtifact`), and the widget-reduction as specified would have broken the existing `/interviews` detail page (still imports the chat pane the spec removes, plus a separate pre-existing crash on multi-sessions).
...
**Not implemented — plan written (2026-09-03).** Seventh of 11. 16 tasks, 92 steps. No blocking questions. Two spec code references were wrong (`TenantToolRegistry` doesn't exist, the class is `ToolRegistry`; `McpArtifactProvider`/`artifact.get` don't back a generic renderer — the provider is diagram-only) — corrected to build `resources/read` on a new shared markdown renderer, consistent with the same finding made independently by the dokumentensicht plan. Protocol version bump is turned into version negotiation (not a hard swap) since `initialize` today ignores `params.protocolVersion` entirely. Depends on `ApiKey.scope` from the ki-vorschlag-als-zustand plan (checked and blocked-not-duplicated in Tasks 10/13).
...
**Not implemented — plan written (2026-09-03).** Tenth of 11. 17 tasks, 85 steps. No blocking questions (two non-blocking, resolved with a documented default). Found the shared markdown renderer the spec assumes exists (`McpArtifactProvider`) is diagram-only — **this plan now builds the shared renderer** (pulled out of `ExportService.export_markdown`) that the mcp-modernisierung plan's `resources/read` consumes, flipping the dependency direction the specs originally documented. Also found the spec's Baseline migration would have been a no-op (`BaselineSnapshot.artifact` is never written anywhere in the current code) and fixed it in the same migration that adds the missing write.

## [2026-09-03-rollenbasierte-sichten.md](2026-09-03-rollenbasierte-sichten.md)
**Not implemented — plan written (2026-09-03).** Eleventh and last of 11. 19 tasks (14 independently runnable against `main`; 3 blocked on the attribute-definition plan, 1 on the dokumentensicht plan). No blocking questions. Found the spec's core problem statement is stale: `NAV_ITEMS.requires` and `useHasRole` already exist in `main` as of commit `54b09760` (the P0-hardening commit that predates this whole spec series) — GitHub issue #848 may already be resolved by that commit and should be checked/closed rather than re-implemented; this plan extends the existing system (adds `approver` to `useHasRole`, reuses an existing `state_meta` field for read-mode locking) instead of rebuilding it.
